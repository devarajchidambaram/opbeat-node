...
test('queue flush isolation', function (t) {
  ...
  var queue = new Queue(function (err, data) {
    t.error(err)
    t.equal(data.app_name, 'app-name')
    t.equal(data.transactions.length, 1, 'should have 1 transaction')
    t.equal(data.transactions[0].traces.length, 0, 'should have 0 traces')

    switch (++flush) {
      case 1:
        t.equal(data.transactions[0].name, 'foo0')
        t.equal(data.transactions[0].type, 'bar0')
        t.equal(data.transactions[0].result, 'baz0')
        break
      case 2:
        t.equal(data.transactions[0].name, 'foo1')
        t.equal(data.transactions[0].type, 'bar1')
