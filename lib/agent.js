...
Agent.prototype.start = function (opts) {
  ...
  if (!this.active) {
    ...
    return this
  } else if (!this.appName || !this.organizationId || !this.secretToken) {
    logger.error('Opbeat isn\'t correctly configured: Missing organizationId, appName or secretToken')
    this.active = false
    return this
  } else {
    debug('agent configured correctly %o', { node: process.version, agent: version, org: this.organizationId, app: this.appName, instrument: this.instrument })
  }
