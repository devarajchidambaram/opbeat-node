...
Agent.prototype.start = function (opts) {
  ...
  if (!this.active) {
    ...
    return this
  } else if (!this.appName || !this.secretToken) {
    logger.error('Opbeat isn\'t correctly configured: Missing appName or secretToken')
    this.active = false
    return this
  } else {
...
Agent.prototype.captureError = function (err, payload, cb) {
  ...
  // populate `tags` context
  payload.tags = objectAssign(
    {},
    trans && trans._tags,
    payload.tags
  )
